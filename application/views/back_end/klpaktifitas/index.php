                                ...
                                <?php if ($records != FALSE): ?>
                                    <?php foreach ($records as $key => $record): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php echo beautify_str($record->kelompok_keterangan) ?>
                                            </td>
                                            <?php if ($access_rules[2][0] == 'allow' || $access_rules[3][0] == 'allow'): ?>
                                                <td class="text-center">
                                                    <div class="btn-group btn-group-sm btn-group-icon">
                                                        <?php if ($access_rules[2][0] == 'allow'): ?>
                                                            <a class="btn btn-default" href="<?php echo base_url("back_end/" . $active_modul . "/detail") . "/" . $record->kelompok_id; ?>"><span class="fa fa-edit"></span></a>
                                                        <?php endif; ?>
                                                        <?php if ($access_rules[3][0] == 'allow'): ?>
                                                            <a class="btn btn-default btn-hapus-row" href="javascript:void(0);" rel="<?php echo base_url("back_end/" . $active_modul . "/delete") . "/" . $record->kelompok_id; ?>"><span class="fa fa-times-circle"></span></a>
                                                            <?php endif; ?>
                                                    </div>
                                                </td>
                                            <?php endif; ?>
