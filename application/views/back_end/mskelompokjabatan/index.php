                                ...
                                <?php if ($records != FALSE): ?>
                                    <?php foreach ($records as $key => $record): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php echo beautify_str($record->keljab_keterangan) ?>
                                            </td>
                                            <?php if ($access_rules[2][0] == 'allow' || $access_rules[3][0] == 'allow'): ?>
                                                <td class="text-center">
                                                    <div class="btn-group btn-group-sm">
                                                        <?php if ($access_rules[2][0] == 'allow'): ?>
                                                            <a class="btn btn-default" href="<?php echo base_url("back_end/" . $active_modul . "/detail") . "/" . $record->keljab_id; ?>">Ubah</a>
                                                        <?php endif; ?>
                                                        <?php if ($access_rules[3][0] == 'allow'): ?>
                                                            <a class="btn btn-default btn-hapus-row" href="javascript:void(0);" rel="<?php echo base_url("back_end/" . $active_modul . "/delete") . "/" . $record->keljab_id; ?>">Hapus</a>
                                                        <?php endif; ?>
                                                    </div>
                                                </td>
                                            <?php endif; ?>
